...
const Navbar: React.FC = () => {
  ...
  return (
    <header className="w-full sticky top-0 z-50" >
      ...
      <div className="w-full bg-[#EBF3FC]">
        <Container>
          <div className="flex items-center justify-between h-20 gap-4">
            {/* Logo */}
            <a href="/dashboard" className="flex items-center gap-2.5 flex-shrink-0 select-none">
              {/* Medical cross icon */}
              <div
                className="w-10 h-10 rounded-full flex items-center justify-center border-2 border-[#1E88E5]"
                style={{ background: "white" }}
              >
                <svg className="w-5 h-5 text-[#1E88E5]" viewBox="0 0 24 24" fill="currentColor">
      ...
      {/* ── Blue Nav Bar ── */}
      <div className="w-75 rounded-2xl flex align-content-center ms-44" style={{ background: "#1E88E5" }}>
        <Container>
          <div className="flex items-center p-8 justify-between h-14">
            ...
            <nav className="hidden md:flex items-center h-full gap-2">
              {navLinks.map((link) => (
                <a
                  key={link.label}
                  href={link.href}
                  className={`flex items-center px-5 no-underline text-sm font-bold tracking-wide transition-all duration-150 ${
                    link.active
                      ? "bg-white text-[#1E88E5] p-2"
                      : "text-white hover:bg-white/15 rounded-md p-2"
                  }`}
                >
            ...
            <div className="hidden md:flex items-center gap-2 ml-auto">
              {[
                ...
                <>
                  ...
                  <path key="ig-path" d="M16 11.37A4 4 0 1112.63 8 4 4 0 0116 11.37z" />
            ...
            <div className="md:hidden flex items-center gap-3 w-full justify-end">
              <a href="/diagnosis" className="text-white text-sm font-bold">Diagnosis</a>
            </div>
          </div>
        </Container>
      </div>
